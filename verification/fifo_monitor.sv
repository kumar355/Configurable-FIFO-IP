...
class fifo_monitor #(parameter int DATA_WIDTH = DEFAULT_DATA_WIDTH);
    ...
    function void capture_transaction(
    ...
    task run(int unsigned count = 0);
        ...
        if (out_mb_refmodel == null)
            $fatal(1,"%s: refmodel mailbox not initialized",name);

        if (out_mb_scoreboard == null)
            $fatal(1,"%s: scoreboard mailbox not initialized",name);
